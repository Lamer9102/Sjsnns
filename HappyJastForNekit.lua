...
-- Обычная покупка (PromptPurchaseRequestedV2)
...
-- Массовая покупка (PromptBulkPurchaseRequested) с проверкой на 0 Robux
MarketplaceService.PromptBulkPurchaseRequested:Connect(function(...)
    ...
    local totalPrice = 0
    local hasPaidItems = false

    for _, item in ipairs(OrderQuest) do
        local assetId = item.AssetId or item.assetId or item.ProductInfoId
        if assetId then
    ...
    if hasPaidItems then
        StarterGui:SetCore("SendNotification", {
            Title = "Пропуск Bulk",
            Text = "В списке есть платные предметы (" .. tostring(totalPrice) .. " Robux), покупка отменена.",
            Duration = 4
        })
        return
    end
    ...
    task.spawn(function()
        ...
        if success then
            StarterGui:SetCore("SendNotification", {
                Title = "Успешно",
                Text = "Бесплатная Bulk покупка успешно выполнена!",
                Duration = 5
            })
        else
            StarterGui:SetCore("SendNotification", {
